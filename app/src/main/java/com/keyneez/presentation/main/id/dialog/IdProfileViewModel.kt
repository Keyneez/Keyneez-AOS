...
@HiltViewModel
class IdProfileViewModel @Inject constructor(
        ...
        viewModelScope.launch {
            ...
            userRepository.getId().onSuccess { response ->
                ...
                if (response.data == null) {
                    Timber.d("GET ID PROFILE DATA IS NULL")
                    _stateMessage.value = UiState.Failure(ID_NULL_CODE)
                    return@onSuccess
                }
                Timber.d("GET ID PROFILE DATA SUCCESS")
                Timber.d("response : $response")
                _userData.value = response.data!!
                _stateMessage.value = UiState.Success
            }.onFailure {
                Timber.e("GET ID PROFILE DATA SERVER ERROR")
